...
- 本地保存与备份：题目列表、模式、排序、当前位置及本轮已提交答案在刷新后保留；笔记、所见草稿、结构化报告、收藏和完成状态都绑定稳定病例 ID。学习进度可导出为 JSON，并在其他设备导入合并。
...
检查 700 个题目与课程一一对应、扩展病例数量、选项和必需字段、图片文件格式、旧病例顺序、686 个来源及文件指纹唯一性，以及本地图片 SHA-256 是否与审计清单逐一一致。新增 200 例另经过联系表目视审查和近似图像哈希筛查。这是来源与数据一致性检查，不代替医学专家审校。
